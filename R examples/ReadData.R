...
# please note that you may have to use either "/" (Linux, Mac OS)
# or "\" (Windows) in your file path
dtaDir <- "NeoCard"
# create a list with a subject / participant codes / IDs
sbjLst <- c("NC_3398-201111Nov14", "NC_3398-201111Nov14", "NC_3409-201111Nov18", "NC_3418-201111Nov17",
            "NC_3428-201111Nov21", "NC_3439-201112Dec02", "NC_3448-201112Dec05", "NC_3458-201112Dec05",
            "NC_3469-201112Dec16", "NC_3479-201112Dec16", "NC_3489-201202Feb24")
# define which item belongs to which NEO-scale and whether the item is normal or inverted
...
# assign NULL to the output data frame
dtaFrm <- NULL

for (crrSbj in sbjLst) {
    # assemble the file name for the current participant
    fleNme <- file.path(dtaDir, paste0(crrSbj, "_NEO-PI.log"))
    # you could also have sprintf("%s_NEO-PI.log", crrSbj) instead of paste0(...)
    
    # read the raw data from the output file, there is no header in the file and the columns 
    # are separated with tabs (\t)
    rawDta <- read.csv(fleNme, header = FALSE, sep = "\t")
    ...
    valVar <- cbind(data.frame(ID = crrSbj), valVar)
...
if (nzchar(system.file(package = "jmvReadWrite"))) {
    ...
    save(list = "dtaFrm", file = paste0(dtaDir, "NeoCard.RData"))
}
