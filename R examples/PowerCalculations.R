...
eta = 0.2
pwr.anova.test(k = 4, f = sqrt(eta / (1 - eta)), sig.level = 0.05, power = 0.8, alternative = "two.sided")

# Power calculations for correlation / regression models
R = 0.3
pwr.r.test(r = R, sig.level = 0.05, power = 0.9, alternative = "greater")

# assigning the sample-size-calculations to an output variable and plotting them
pwr_t = pwr.t.test(d = 0.5, sig.level = 0.05, power = 0.9, type="two.sample", alternative="greater")
...
pwr.t.test(d = cohen.ES(test="t", size="medium")$effect.size, sig.level = 0.05, power = 0.9, type="two.sample", alternative="greater")
